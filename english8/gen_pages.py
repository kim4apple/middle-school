#!/usr/bin/env python3
"""Generate Grade 8 English unit pages from content data."""

import os, sys, importlib
sys.path.insert(0, os.path.dirname(__file__))

GRADE = "english8"
THEME_KEY = "eng8-theme"
CSS_FILE = "css/english8.css"

# Content modules: each defines UNITS_xx dict
CONTENT_MODULES = ["content_8a", "content_8b"]

ALL_UNITS = [
    ("u01","U1","Happy Holiday!", "假期活动",None,"u02.html"),
    ("u02","U2","How often do you exercise?", "生活习惯","u01.html","u03.html"),
    ("u03","U3","I'm more outgoing than my sister.", "个人特质","u02.html","u04.html"),
    ("u04","U4","What's the best movie theater?", "偏好比较","u03.html","u05.html"),
    ("u05","U5","Do you want to watch a game show?", "媒体偏好","u04.html","u06.html"),
    ("u06","U6","I'm going to study computer science.", "职业规划","u05.html","u07.html"),
    ("u07","U7","Will people have robots?", "未来预测","u06.html","u08.html"),
    ("u08","U8","Let's Communicate!", "沟通表达","u07.html",None),
    ("b2u01","U1","Time to Relax!", "放松与娱乐",None,"b2u02.html"),
    ("b2u02","U2","Let's clean up!", "环保行动","b2u01.html","b2u03.html"),
    ("b2u03","U3","Could you please clean your room?", "家务劳动","b2u02.html","b2u04.html"),
    ("b2u04","U4","Why don't you talk to your parents?", "人际沟通","b2u03.html","b2u05.html"),
    ("b2u05","U5","What were you doing when the rainstorm came?", "过去进行","b2u04.html","b2u06.html"),
    ("b2u06","U6","An old man tried to move the mountains.", "故事传说","b2u05.html","b2u07.html"),
    ("b2u07","U7","What's the highest mountain in the world?", "世界之最","b2u06.html","b2u08.html"),
    ("b2u08","U8","Making a Difference", "社会责任","b2u07.html",None),
]

def load_content():
    """Load all content modules into a single dict keyed by unit id."""
    merged = {}
    for mod_name in CONTENT_MODULES:
        try:
            ...
            attr_names = [f"UNITS_{suffix}", f"UNITS_{suffix.upper()}"]
            attr = None
            for n in attr_names:
                if hasattr(mod, n):
                    attr = getattr(mod, n)
                    break
            if attr is None:
                print(f"  ⚠ Could not find UNITS_* in {mod_name}")
                continue
...
def build_sidebar(current_id):
    ...
    lines.append('        <h2 style="margin-bottom:0;">英语八年级</h2>')
    ...
    lines.append('        <a href="#" style="font-size:12px;color:var(--text-secondary);margin:4px 0;">━━ 八 上 ━━</a>')
    for uid, num, title, _, _, _ in s1:
        active_mark = ' class="active"' if uid == current_id else ''
        lines.append(f'        <a href="{uid}.html"{active_mark}>{num} {title}</a>')
    lines.append('        <a href="#" style="font-size:12px;color:var(--text-secondary);margin:4px 0;">━━ 八 下 ━━</a>')
    for uid, num, title, _, _, _ in s2:
        active_mark = ' class="active"' if uid == current_id else ''
        lines.append(f'        <a href="{uid}.html"{active_mark}>{num} {title}</a>')
    lines.append('      </nav>')
...
def build_exam_qs(qs, tag, diff_prefix):
    questions = []
    for item in qs:
...
def build_page(uid, num, title, topic, prev, next_, c):
    page_title = f"{num} {title} | 初中英语八年级"
...
<details class="exam-module" open><summary>📝 三级闯关 · 实战练习</summary><div class="content"><h3 class="level-basic">🥉 基础关 · 概念识记</h3>{build_exam_qs(c["basic_qs"],"基础","")}<h3 class="level-mid">🥈 提升关 · 语境运用</h3>{build_exam_qs(c["mid_qs"],"提升","")}<h3 class="level-hard">🥇 挑战关 · 综合运用</h3>{build_exam_qs(c["hard_qs"],"挑战","")}</div></details>
...
<div class="next-stop" style="margin-top:8px;"><strong>➡️ 下一单元：{c.get("next_unit_title","")}</strong><br>{next_unit_text}</div>
</main>
